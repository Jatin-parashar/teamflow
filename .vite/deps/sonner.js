"use client";
import { require_react_dom } from "./chunk-ADBY2UQ7.js";
import { require_react } from "./chunk-RTNHS3CA.js";
...
var useIsDocumentHidden = () => {
  const [isDocumentHidden, setIsDocumentHidden] = import_react.default.useState(
    document.hidden
  );
  ...
  constructor() {
      ...
      const originalPromise = p
        .then(async (response) => {
          ...
          const isReactElementResponse =
            import_react.default.isValidElement(response);
          if (isReactElementResponse) {
            ...
            const isExtendedResult =
              ...
              !import_react.default.isValidElement(promiseData);
    ...
    this.getActiveToasts = () => {
      return this.toasts.filter(
        (toast2) => !this.dismissedToasts.has(toast2.id)
      );
    };
    this.subscribers = [];
    this.toasts = [];
...
var Toast = (props) => {
  ...
  const [swipeDirection, setSwipeDirection] =
    import_react.default.useState(null);
  ...
  const [offsetBeforeRemove, setOffsetBeforeRemove] =
    import_react.default.useState(0);
  ...
  import_react.default.useLayoutEffect(() => {
    ...
    setHeights((heights2) => {
      const alreadyExists = heights2.find(
        (height) => height.toastId === toast2.id
      );
      if (!alreadyExists) {
        return [
          {
  ...
  import_react.default.useEffect(() => {
    ...
    const startTimer = () => {
      ...
      timeoutId = setTimeout(() => {
        toast2.onAutoClose == null
          ? void 0
          : toast2.onAutoClose.call(toast2, toast2);
        deleteToast();
  ...
  return import_react.default.createElement(
    "li",
    {
      ...
      "data-sonner-toast": "",
      "data-rich-colors":
        (_toast_richColors = toast2.richColors) != null
          ? _toast_richColors
          : defaultRichColors,
      ...
      onPointerUp: () => {
        ...
        if (Math.abs(swipeAmount) >= SWIPE_THRESHOLD || velocity > 0.11) {
          setOffsetBeforeRemove(offset.current);
          toast2.onDismiss == null
            ? void 0
            : toast2.onDismiss.call(toast2, toast2);
          if (swipeDirection === "x") {
    ...
    import_react.default.createElement(
      ...
      toast2.description
        ? import_react.default.createElement(
            ...
            typeof toast2.description === "function"
              ? toast2.description()
              : toast2.description
          )
        : null
  ...
  [defaultOffset, mobileOffset].forEach((offset, index) => {
    ...
    function assignAll(offset2) {
      ["top", "right", "bottom", "left"].forEach((key) => {
        styles[`${prefix}-${key}`] =
          typeof offset2 === "number" ? `${offset2}px` : offset2;
      });
    }
        ...
        setTimeout(() => {
          import_react_dom.default.flushSync(() => {
            setActiveToasts((toasts) =>
              toasts.filter((t) => t.id !== toast2.id)
            );
          });
        });
        return;
    ...
    setToasts((toasts2) => {
      ...
      if (
        !((_toasts_find = toasts2.find(
          (toast2) => toast2.id === toastToRemove.id
        )) == null
          ? void 0
          : _toasts_find.delete)
      ) {
      ...
      possiblePositions.map((position2, index) => {
        ...
        return import_react.default.createElement(
          ...
          toasts
            .filter(
              (toast2) =>
                (!toast2.position && index === 0) ||
                toast2.position === position2
            )
            .map((toast2, index2) => {
