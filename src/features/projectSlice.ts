...
>("projects/fetchProjectById", async (projectId, { rejectWithValue }) => {
  try {
    const data = await firebaseFetch<Omit<Project, "id"> | null>(
...
>(
  ...
  async ({ projectId, member }, { rejectWithValue }) => {
    try {
      const projectData = await firebaseFetch<Omit<Project, "id"> | null>(
        `projects/${projectId}.json`
      );
      ...
      if (
        existingMembers.some((m: ProjectMember) => m.userId === member.userId)
      ) {
...
>(
  ...
  async ({ projectId, userId }, { rejectWithValue }) => {
    try {
      ...
      const updatedMembers = (projectData.members || []).filter(
        (m: ProjectMember) => m.userId !== userId
      );
