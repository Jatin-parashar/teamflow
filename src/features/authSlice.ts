import { getCurrentAuthUser, logIn, logOut, signUp } from '@/firebase/firebaseAuth';
...
export const initializeAuth = createAsyncThunk<User | null, void, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        try {
            ...
            if (firebaseUser) {
                const userData = await firebaseFetch<Omit<User, 'id'> | null>(`users/${firebaseUser.uid}.json`);
...
export const login = createAsyncThunk<User, { email: string; password: string }, { rejectValue: string }>(
    ...
    async ({ email, password }, { rejectWithValue }) => {
        try {
            ...
            const userData = await firebaseFetch<Omit<User, 'id'> | null>(`users/${uid}.json`);
