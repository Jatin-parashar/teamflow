...
import { RequestStatus, type TaskPriority, type TaskStatus } from './types';
...
export const fetchTasks = createAsyncThunk<Task[], void, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const data = await firebaseFetch<Record<string, Omit<Task, 'id'>> | null>('tasks.json');
            if (!data) return [];
...
export const fetchTasksByProject = createAsyncThunk<Task[], string, { rejectValue: string }>(
...
export const fetchTasksByUser = createAsyncThunk<Task[], string, { rejectValue: string }>(
...
export const fetchTaskById = createAsyncThunk<Task, string, { rejectValue: string }>(
    ...
    async (taskId, { rejectWithValue }) => {
        try {
            const data = await firebaseFetch<Omit<Task, 'id'> | null>(`tasks/${taskId}.json`);
...
export const createTask = createAsyncThunk<Task, Omit<Task, 'id' | 'createdAt' | 'updatedAt'>, { rejectValue: string }>(
    'tasks/createTask',
...
const taskSlice = createSlice({
    ...
    reducers: {
        ...
        optimisticUpdateTaskStatus: (state, action: PayloadAction<{ taskId: string; status: TaskStatus }>) => {
...
export const { setCurrentTask, clearTaskError, optimisticUpdateTaskStatus } = taskSlice.actions;
