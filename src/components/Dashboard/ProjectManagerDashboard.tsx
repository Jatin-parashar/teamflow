...
const ProjectManagerDashboard = () => {
  ...
  const taskStats = {
    ...
    done: myProjectTasks.filter((t) => t.status === TaskStatus.DONE).length,
    blocked: myProjectTasks.filter((t) => t.status === TaskStatus.BLOCKED)
      .length,
    inProgress: myProjectTasks.filter(
      (t) => t.status === TaskStatus.IN_PROGRESS
    ).length,
    overdue: myProjectTasks.filter(
  ...
  const projectStats = {
    ...
    inProgress: myProjects.filter((p) => p.status === ProjectStatus.IN_PROGRESS)
      .length,
    completed: myProjects.filter((p) => p.status === ProjectStatus.COMPLETED)
      .length,
    onHold: myProjects.filter((p) => p.status === ProjectStatus.ON_HOLD).length,
  };
                ...
                <div className="flex items-center gap-2">
                  <div className="w-24 bg-secondary rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${taskStats.total > 0 ? (taskStats.blocked / taskStats.total) * 100 : 0}%`,
                      }}
                    />
                  </div>
                  <span className="text-sm font-medium">
